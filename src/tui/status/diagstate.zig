...
fn format(self: *Self) void {
    ...
    if (self.errors > 0) fbs.print("  {d}", .{self.errors}) catch {};
    if (self.warnings > 0) fbs.print("  {d}", .{self.warnings}) catch {};
    if (self.info > 0) fbs.print("  {d}", .{self.info}) catch {};
    if (self.hints > 0) fbs.print("  {d}", .{self.hints}) catch {};
